...
| `vaccine_stockpile` | No | Empty list | Dose releases. Each entry requires an integer simulation `day` and a dose `amount`. Entries with the same effective day are combined. |
...
people between disease compartments. Vaccine effectiveness is then applied by
the disease and travel models.
...
parameters control what happens after treatment:
